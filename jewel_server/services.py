...
def post_sale(conn,payload,user,client_ip=None):
    ...
    if cid and credit:
        row=c.execute('SELECT balance_paise FROM customers WHERE id=?',(cid,)).fetchone();newp=int(row['balance_paise'] or 0)+money_paise(credit);c.execute('UPDATE customers SET balance=?,balance_paise=?,updated_at=? WHERE id=?',(money(newp/100),newp,now,cid))
    jl=[]
...
def cancel_sale(conn,sale_id,user,reason='',client_ip=None):
    ...
    if sale['customer_id'] and sale['payment_credit']:
        row=conn.execute('SELECT balance_paise FROM customers WHERE id=?',(sale['customer_id'],)).fetchone();newp=int(row['balance_paise'] or 0)-money_paise(sale['payment_credit']);conn.execute('UPDATE customers SET balance=?,balance_paise=?,updated_at=? WHERE id=?',(money(newp/100),newp,now,sale['customer_id']))
